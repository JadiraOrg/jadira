...
import org.jadira.usertype.jsr310.testmodel.jodatime.LocalTimeAsTimeJoda;
import org.jadira.usertype.jsr310.testmodel.jodatime.TimeOfDayAsTimeJoda;
import org.jadira.usertype.jsr310.testmodel.jsr310.LocalTimeHolder;
...
public class TestPersistentLocalTime extends DatabaseCapable {
    ...
    @Test
    @Ignore // This test will fail due to Joda Time defect
    public void testNanosWithJodaTime() {
        ...
        LocalTimeAsTimeJoda jodaItem = manager.find(LocalTimeAsTimeJoda.class, Long.valueOf(1));

        assertNotNull(jodaItem);
        assertEquals(1, jodaItem.getId());
        assertEquals("test_nanos1", jodaItem.getName());
        assertEquals(new org.joda.time.LocalTime(10, 10, 10, 0), jodaItem.getLocalTime());
    ...
    public void testRoundtripWithJodaTimeOfDay() {
        ...
        manager.getTransaction().begin();
        for (int i = 0; i < localTimes.length; i++) {
            LocalTimeHolder entity = manager.find(LocalTimeHolder.class, Long.valueOf(i));
            if (entity != null) {
