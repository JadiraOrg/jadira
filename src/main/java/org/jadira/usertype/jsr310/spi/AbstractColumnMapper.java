...
public abstract class AbstractColumnMapper<T, J> implements Serializable, ColumnMapper<T, J> {
    ...
    /* (non-Javadoc)
     * @see org.jadira.usertype.jsr310.spi.ColumnMapper#returnedClass()
     */
    public final Class<T> returnedClass() {
        @SuppressWarnings("unchecked")Class<T> result = (Class<T>) TypeHelper.getTypeArguments(ColumnMapper.class, getClass()).get(0);
        return result;
    }
    
    /* (non-Javadoc)
     * @see org.jadira.usertype.jsr310.spi.ColumnMapper#jdbcClass()
     */
