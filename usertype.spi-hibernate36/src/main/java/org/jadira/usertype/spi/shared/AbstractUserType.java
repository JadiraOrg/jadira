...
public abstract class AbstractUserType implements Serializable {
    ...
    public void beforeNullSafeOperation(SessionImplementor session) {
    	...
    	SessionFactoryImplementor impl = session == null ? null : session.getFactory();    	
    	ConfigurationHelper.setCurrentSessionFactory(impl);
    	if (this instanceof IntegratorConfiguredType) {
    		((IntegratorConfiguredType)this).applyConfiguration(impl);
    	}
    }
