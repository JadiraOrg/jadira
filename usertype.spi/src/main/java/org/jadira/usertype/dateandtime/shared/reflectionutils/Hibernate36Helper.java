...
public class Hibernate36Helper {
	...
	public static Object nullSafeGet(ColumnMapper<?, ?> mapper,
		...
		try {
			...
			throw new ReflectionException(
					"Problem invoking nullSafeGet Method: " + e.getMessage(), e);
		} catch (IllegalAccessException e) {
			throw new ReflectionException(
					"Problem invoking nullSafeGet Method: " + e.getMessage(), e);
	...
	public static void nullSafeSet(ColumnMapper<?, ?> mapper,
		...
		try {
			NULL_SAFE_SET_METHOD.invoke(mapper.getHibernateType(),
					new Object[] { preparedStatement, object, index,
							NO_WRAPPER_OPTIONS });
		} catch (IllegalArgumentException e) {
			throw new ReflectionException(
					"Problem invoking nullSafeGet Method: " + e.getMessage(), e);
	...
	private static Class<?> getAbstractStandardBasicTypeClass() {
		...
		try {
			return Class
					.forName("org.hibernate.type.AbstractStandardBasicType");
	...
	private static Class<?> getAbstractSingleColumnStandardBasicTypeClass() {
		...
		try {
			return Class
					.forName("org.hibernate.type.AbstractSingleColumnStandardBasicType");
		} catch (ClassNotFoundException e) {
			return null;
		}
	}
	...
	private static Class<?> getWrapperOptionsClass() {
		...
		try {
			return Class
					.forName("org.hibernate.type.descriptor.WrapperOptions");
		} catch (ClassNotFoundException e) {
			return null;
		}
	}
	...
	private static Method getNullSafeGetMethod() {
		...
		try {
			Method method = ABSTRACT_STANDARD_BASIC_TYPE_CLASS
					.getDeclaredMethod("nullSafeGet", new Class[] {
							ResultSet.class, String.class,
							WRAPPER_OPTIONS_CLASS });
			method.setAccessible(true);

			return method;
		} catch (SecurityException e) {
			throw new ReflectionException(
					"Problem retrieving nullSafeGet Method: " + e.getMessage(),
					e);
	...
	private static Method getNullSafeSetMethod() {
		...
		try {
			Method method = ABSTRACT_STANDARD_BASIC_TYPE_CLASS
					.getDeclaredMethod("nullSafeSet", new Class[] {
							PreparedStatement.class, Object.class,
							Integer.TYPE, WRAPPER_OPTIONS_CLASS });
			method.setAccessible(true);
	...
	private static Method getToStringMethod() {
		...
		try {
			...
			throw new ReflectionException(
					"Problem retrieving toString Method: " + e.getMessage(), e);
		} catch (NoSuchMethodException e) {
			throw new ReflectionException(
					"Problem retrieving toString Method: " + e.getMessage(), e);
		}
	...
	private static Object getNoWrapperOptions() {
		...
		try {
			...
			throw new ReflectionException(
					"Problem retrieving NoWrapperOptions: " + e.getMessage(), e);
		} catch (NoSuchFieldException e) {
			throw new ReflectionException(
					"Problem retrieving NoWrapperOptions: " + e.getMessage(), e);
		} catch (IllegalArgumentException e) {
