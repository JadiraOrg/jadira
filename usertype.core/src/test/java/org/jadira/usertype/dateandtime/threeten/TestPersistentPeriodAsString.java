...
import org.jadira.usertype.dateandtime.threeten.testmodel.PeriodAsStringJdk8;
...
public class TestPersistentPeriodAsString extends DatabaseCapable {
    ...
    public void testPersist() throws SQLException, IOException {
        ...
        for (int i = 0; i < periods.length; i++) {
            PeriodAsStringJdk8 item = new PeriodAsStringJdk8();
            item.setId(i);
        ...
        for (int i = 0; i < periods.length; i++) {
            ...
            PeriodAsStringJdk8 item = manager.find(PeriodAsStringJdk8.class, Long.valueOf(i));
        ...
        verifyDatabaseTable(manager, PeriodAsStringJdk8.class.getAnnotation(Table.class).name());
    ...
    public void testRoundtripWithJodaTime() {
        ...
        manager.getTransaction().begin();
        for (int i = 0; i < periods.length; i++) {
            manager.remove(manager.find(PeriodAsStringJdk8.class, Long.valueOf(i)));
        }
        manager.flush();
        manager.getTransaction().commit();
